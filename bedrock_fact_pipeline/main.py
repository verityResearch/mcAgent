...
def _parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Minecraft Bedrock Fact Pipeline")
    ...
    parser.add_argument("--output", type=str, default="bedrock_fact_pipeline/data/verified/dataset.jsonl",
                        help="Output JSONL path")
    parser.add_argument("--data-report-dir", type=str, required=True,
                        help="Path to the extract_pinned_version output directory (the extracted "
                             "bedrock-samples tree)")
...
def _next_batch_composition(batch_size, fact_seeded_ratio, fact_seeded_dispatched, total_dispatched):
    ...
    Ported from fact_pipeline/main.py (same bug, identical code, found while
    fact_pipeline's copy was being fixed -- see mcAgent PR #8).
    """
...
async def _run(args, teacher_api_key, judge_api_key):
    ...
    orchestrator = PipelineOrchestrator(teacher, check_fact_seeded, judge, concurrency_limit=args.concurrency)
