...
def test_convert_to_jsonl_cleans_up_and_leaves_no_file_when_a_row_is_malformed(tmp_path):
    # An admitted sample missing a required key raises mid-write; the atomic
    # write must remove the .tmp sidecar and never publish output_path.
    results = [
        {"lane": "fact_seeded", "sample": {"instruction": "q1", "reasoning": "r1", "answer": "a1"},
         "verification": {"success": True, "errors": [], "failure_kind": None}},
        {"lane": "free_recall", "sample": {"instruction": "q2", "reasoning": "r2"},  # no "answer"
         "verification": {"success": True, "errors": [], "failure_kind": None}},
    ]
    output_path = str(tmp_path / "out.jsonl")

    with pytest.raises(KeyError):
        convert_to_jsonl(results, output_path)
