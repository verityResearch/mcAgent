...
- **Knowledge questions:** an early, loosely scored run credited the v20 adapter with 104/106 matching lookups on held-out items and claimed it beat the base model. That claim was **retracted**: the base model was never shown the tool interface, and the parser accepted a lookup anywhere in the text. A fair paired rerun then gave **both** models the same tool manifest and scored a strict tool-call contract: on 106 held-out questions the base model made **0** correct calls and the v20 adapter **23**. All 23 are questions the base failed, and none went the other way (exact McNemar test). Adversarial rephrasings: base 0/43, v20 2/43. So the adapter learned something real, but it is weak in absolute terms. The drop from 104 to 23 can't be pinned on one cause: the scoring became strict, and the evaluator now adds a manifest that v20 never saw in training. See [the measurement correction](docs/reports/2026-09-03-v20-evaluation-measurement-correction.md) and [the claim ledger](docs/reports/2026-09-02-benchmark-claim-ledger.md).
